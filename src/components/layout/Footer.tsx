...
export default function Footer() {
  return (
    <footer className="bg-[var(--bg-secondary)] border-t border-[var(--border-subtle)] relative overflow-hidden text-[var(--text-secondary)] transition-colors duration-300">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-20 pb-12">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-10 lg:gap-8 pb-16 border-b border-[var(--border-subtle)]">
          
          {/* Brand Column (Span 2) */}
          <div className="lg:col-span-2 space-y-4">
            <Link href="/" className="inline-flex items-center group transition-opacity hover:opacity-90">
              <BrandLogo variant="auto" size="md" showWordmark={true} showDescriptor={false} />
            </Link>

            <p className="text-[11px] uppercase font-mono tracking-[0.2em] text-[var(--accent)]">
            ...
            <div className="pt-2 text-xs font-mono text-[var(--text-primary)]">
              <span className="text-[var(--text-muted)]">Core Thesis: </span>BUILD. CONNECT. INTELLIGENCE.
            </div>

            <div className="pt-1 text-[11px] text-[var(--text-muted)] font-mono">
          ...
          {/* Column 1: Solutions */}
          <div>
            <h4 className="text-[11px] uppercase font-mono tracking-widest text-[var(--text-primary)] font-medium mb-4">
              Solutions
            </h4>
              ...
              {SOLUTIONS.map((sol) => (
                <li key={sol.id}>
                  <Link
                    ...
                    className="hover:text-[var(--text-primary)] transition-colors flex items-center group"
                  >
                    <span>{sol.name}</span>
                    <ArrowUpRight className="w-2.5 h-2.5 ml-1 opacity-0 -translate-x-1 group-hover:opacity-100 group-hover:translate-x-0 transition-all text-[var(--accent)]" />
                  </Link>
                </li>
              ))}
            </ul>
          ...
          <div>
            ...
            <ul className="space-y-2 text-xs">
              {INDUSTRIES.slice(0, 5).map((ind) => (
                <li key={ind.name}>
                  <Link href="/industries" className="hover:text-[var(--text-primary)] transition-colors">
                    {ind.name}
                  </Link>
                </li>
              ))}
              <li>
                <Link href="/industries" className="text-[var(--accent)] hover:underline font-medium inline-flex items-center gap-1">
                  All 12 Sectors →
                </Link>
              </li>
            </ul>
          ...
          <div>
            ...
            <ul className="space-y-2 text-xs">
              <li>
                <Link href="/#architecture" className="hover:text-[var(--text-primary)] transition-colors">
                  Capabilities Stack
                </Link>
              </li>
              <li>
                <Link href="/insights" className="hover:text-[var(--text-primary)] transition-colors">
                  ARKAPRANA Insights
                </Link>
              </li>
              <li>
